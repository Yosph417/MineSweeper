...
//https://www.youtube.com/watch?v=UA3dQIqBgjc
...
public class Grid {
	...
	public boolean[][] getBombGrid(){
		boolean[][] tempBombGrid = new boolean[numRows][numColumns];
		for(int i = 0; i < numRows; ++i) {
			for(int o = 0; o < numColumns; ++o) {
				tempBombGrid[i][o] = bombGrid[i][o];
			}
		}
		return tempBombGrid;
	}
	...
	public int[][] getCountGrid(){
		int[][] tempCountGrid = new int[numRows][numColumns];
		for(int i = 0; i < numRows; ++i) {
			for(int o = 0; o < numColumns; ++o) {
				tempCountGrid[i][o] = countGrid[i][o];
			}
		}
	...
	public void createBombGrid() {
		int currentBombNumber = 0;
		Random ranNum = new Random();
		int ranRow;
		int ranColumn;
		while(currentBombNumber < numBombs) {
			ranRow = ranNum.nextInt(numRows);
			ranColumn = ranNum.nextInt(numColumns);
			if(!isBombAtLocation(ranRow, ranColumn)) {
				bombGrid[ranRow][ranColumn] = true;
				++currentBombNumber;
			}
		}
	...
	private void createCountGrid() {
		for(int i = 0; i < numRows; ++i) {
			...
			for (int o = n - 1; o <= n+1; ++o) {
				if(arrayInBounds(m,o)) {
					if(isBombAtLocation(m,o)) {
						countGrid[i][n]++;
					}
				}
			}
	...
	public boolean arrayInBounds(int numRows, int numColumns) {
		if(numRows >= 0 && numRows < this.numRows) {
			if(numColumns >= 0 && numColumns < this.numColumns) {
				return true;
			}
		}
				return false;
